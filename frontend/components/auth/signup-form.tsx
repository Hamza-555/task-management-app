...
export function SignupForm() {
  ...
  const apiError =
    signup.error && "response" in signup.error
      ? (signup.error as any).response?.data?.error
      : null;

  const apiDetails =
    signup.error && "response" in signup.error
      ? (signup.error as any).response?.data?.details
      : null;
  ...
  return (
    ...
    >
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">Start managing your tasks for free</p>
      </div>
      ...
      <form onSubmit={handleSubmit((d) => signup.mutate(d))} className="space-y-4">
        ...
        <Field label="Email" error={errors.email?.message ?? apiDetails?.email}>
          <input
            {...register("email")}
            type="email"
            placeholder="you@example.com"
            autoComplete="email"
            className={inputClass(!!errors.email || !!apiDetails?.email)}
          />
        </Field>
